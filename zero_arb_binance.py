...
# os and file related definitions
...
while(True):
	...
	if len(trades_df) != 0: 
		...
		print('complete_trades_df', complete_trades_df)
		...
		for i, row in complete_trades_df.iterrows():
		    ...
		    if abs(timestamp - timestamp_now) < 1000: 
		        if trade_id in trades_analyzed:
		        	...
		        	complete_trades_df.iloc[i,19] = 'repeat'
		            ...
		            complete_trades_df.iloc[i, 18], complete_trades_df.iloc[i,19] = row_binance(sell_token_symbol, buy_token_symbol, timestamp, sell_token_qty, buy_token_qty)
		            trades_analyzed.add(trade_id)
 
		            
		    else:
		        complete_trades_df.iloc[i,18] = 'Timeout'
		        complete_trades_df.iloc[i,19] = 'Timeout'
